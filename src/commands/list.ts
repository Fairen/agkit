...
export async function listCommand(): Promise<void> {
  ...
  p.log.message(
    `Remote and local templates are also supported:\n  souk add gh:owner/repo/dir my-plugin\n  souk add gl:owner/repo#v2 my-plugin\n  souk add https://git.company.io/team/templates.git//skill-fr my-plugin\n  souk add ./local-template my-plugin`,
  );
  p.outro("Usage: souk add <template|spec> <plugin-name>");
}
