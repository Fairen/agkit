...
export async function validateCommand(
          ...
          if (!fs.existsSync(manifestPath)) {
            ...
            try {
              ...
              if (manifest.name && manifest.name !== entry.name) {
                findings.push({
                  level: "warn",
                  message: `Catalog entry "${label}" but plugin.json declares "${manifest.name}" — run \`souk sync\``,
                });
              }
              if (
                manifest.version &&
                ...
                findings.push({
                  level: "warn",
                  message: `Version drift for "${label}": catalog ${entry.version} vs plugin.json ${manifest.version} — run \`souk sync\``,
                });
              }
            } catch {
              findings.push({
  ...
  // 4b. Target agents that need generation (recorded but not built by souk).
    ...
    if (needBuild.length > 0) {
      p.log.info(
        `metadata.targets includes tier 2/3 agent(s): ${needBuild.join(", ")}. ` +
          "The Claude catalog validates here; run `souk build` to (re)generate their registries and `souk build --check` in CI.",
      );
    }
  }
